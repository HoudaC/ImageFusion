...
from training import condionnal_train
...
img_train_all = read_data(data_train_dir,500, only_sentinel=False)
img_val_all = read_data(data_val_dir,100, only_sentinel=False)
...
train_dataloader = sr_dataloader(img_train_all, patch, strides, train_dataloader_batch_size, only_sentinel= False,applytransform=True)
val_dataloader = sr_dataloader(img_val_all, patch, strides, train_dataloader_batch_size, only_sentinel= False)


# Initialize the Conditional SRCNN model
model = ConditionalSRCNN(input_channels=14, output_channels=10)
...
condionnal_train(model, train_dataloader, val_dataloader, criterion, optimizer, num_epochs=1000, save_path="weights/best_model_cond.pth", checkpoint_save_path= "weights/last_model_cond.pth",scheduler=scheduler)
